...
//
extern uint8_t _start_heap[];
extern uint8_t _end_heap[];
static const uint8_t *const pool = &_start_heap[0];
#define POOL_SIZE (size_t)(&_end_heap[0] - &_start_heap[0])
...
void *malloc(size_t nbytes)
{
	...
	// sure that if nbytes is a multiple of nquantas, we don't allocate too
	// much
	//
	size_t nquantas =
	...
	for (p = prevp->s.next;; prevp = p, p = p->s.next) {
		...
		if (p->s.size >= nquantas) {
			// exactly ?
			if (p->s.size == nquantas) {
				// just eliminate this block from the free list
				// by pointing its prev's next to its next
				//
